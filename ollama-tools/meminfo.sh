...
# ==============================================================================
# 名称: meminfo
# 描述: 深度监控 AMD GPU 资源。包含 VRAM/GTT 汇总表及进程级显存穿透探测。
# ==============================================================================
...
# 1. 获取 GPU 硬件层面的真实占用
GPU_SNAPSHOT=$(rocm-smi --showmeminfo vram gtt --json 2>/dev/null)
...
echo "$GPU_SNAPSHOT" | jq -r '
  ...
  (sn(."GTT Total Memory (B)")) as $g_t | (sn(."GTT Total Used Memory (B)")) as $g_u |
  def gb(x): (x / 1073741824 | . * 100 | round / 100 | tostring + " GB");
  (
    [$gpu, "VRAM", gb($v_t), gb($v_u), gb($v_t - $v_u)],
    [$gpu, "GTT", gb($g_t), gb($g_u), gb($g_t - $g_u)],
    ["SUM", "Total_Pool", gb($v_t + $g_t), gb($v_u + $g_u), gb(($v_t + $g_t) - ($v_u + $g_u))]
  ) | @tsv' | awk -F'\t' '{printf "%-10s %-12s %-12s %-12s %-12s\n", $1, $2, $3, $4, $5}'
...
for pid in $pids; do
    ...
    V_SMI=0; G_SMI=0; V_KFD=0; V_MAPS=0
    ...
    [ "$VERBOSE" = true ] && echo "[DEBUG] PID $pid: SMI=${V_SMI:-0}, KFD=${V_KFD:-0}, MAPS=${V_MAPS:-0} -> Max: $FINAL_V"
...
if [ "$(echo "$DIFF_B > 1048576" | bc)" -eq 1 ]; then
