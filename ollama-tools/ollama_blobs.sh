...
# 使用大括号包裹循环，最后统一排序去重
{
    find "$MANIFEST_ROOT" -type f 2>/dev/null | while read -r file; do
        ...
        # 3. 提取 Model 类型的 Blob (通过 mediaType 过滤)
        # 逻辑：查找包含 .model 的行，并提取其上一行或本行的 digest
        raw_blob=$(grep -B 2 'image.model' "$file" | grep -oP '"digest":"\K[^"]+' | head -n 1 | sed 's/:/-/g')
        
        # 4. 保底：如果没找到特定的 model 类型，取第一个 digest
        if [ -z "$raw_blob" ]; then
